...
In order to interpolate radiance interval inside one such a cascade, bilateral interpolation is used in space (so that only adjactent probes with similar depth contribute) and bilinear interpolation is used in direction.
...
World space radiance intervals can be calculated in many different ways, including volume raymarching, raytracing (software and hardware): any acceleration structure can work, as long as it allows querying ray segments. The implementation that this work uses signed distance field raymarching purely because of convenience.
...
\begin{figure}[htb]
  ...
  \caption{\label{fig:sdf_world_space}
     A demo of world space radiance intervals encoded in screenspace probes. Images demonstrate the possibility of capturing off-screen world-space lighting and occlusion information in screenspace probes. Each frame takes about 30-50ms to calculate on GTX3060 and each novel view takes 0.1-0.5s to converge due to temporal accumulation(exact time depends on fractal used).}
\end{figure}

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.49\columnwidth]{images/sdf_stress_tiny.png}
  \includegraphics[width=0.49\columnwidth]{images/sdf_stress_many.png}
  \caption{\label{fig:sdf_stress_test}
     Left: stress test with small indirect light sources, Right: stress test with many light sources. Indirect light even from small emissive surfaces can be observed and performance of the method does not depend on their number.}
\end{figure}
\clearpage
\section{Limitations}
